...
export default async function handler(req) {
  ...
  // Reconstruct original bebee URL
  ...
  try {
    ...
    const validThrough = jobPosting?.validThrough || "2026-12-31";
    const empType     = Array.isArray(jobPosting?.employmentType) ? jobPosting.employmentType.join(", ") : (jobPosting?.employmentType || "");
    ...
    const salary      = jobPosting?.baseSalary?.value
      ? `${jobPosting.baseSalary.currency || "USD"} ${jobPosting.baseSalary.value.minValue || jobPosting.baseSalary.value.value || ""}${jobPosting.baseSalary.value.maxValue ? "–" + jobPosting.baseSalary.value.maxValue : ""} / ${(jobPosting.baseSalary.value.unitText || "").toLowerCase()}`
      : "";

    // Updated schema for our domain
    const schema = {
      ...
      "jobLocation": {
        ...
        "address": {
          "@type": "PostalAddress",
          "addressLocality": addr.addressLocality || "United States",
          "addressRegion": addr.addressRegion || "",
          "addressCountry": addr.addressCountry || "US"
        }
      },
...
.section{background:var(--bg2);border:1px solid var(--bor);border-radius:3px;padding:28px 28px;margin-bottom:16px}
...
.desc li{margin-left:20px;margin-bottom:6px}
.schema-pre{font-family:monospace;font-size:.72rem;color:var(--g);line-height:1.6;overflow-x:auto;white-space:pre-wrap;word-break:break-word}
...
<div class="wrap">
  ...
  <div class="apply-bar">
    <a class="apply-btn" href="${bebeeUrl}" target="_blank" rel="noopener">
      Apply Now →
    </a>
  ...
  <div class="section">
    <div class="section-title">JobPosting Schema (JSON-LD)</div>
    <pre class="schema-pre">${JSON.stringify(schema, null, 2)}</pre>
  </div>
</div>
